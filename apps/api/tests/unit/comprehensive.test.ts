 ...
 * 
 * This test suite covers the core requirements for task 20:
 * - PII masking edge cases (Requirement 8.1)
 * - Knowledge base integration (Requirement 8.2) 
 * - Guardrail intervention scenarios (Requirement 8.3)
 * - Empty citations and "Insufficient basis" templates (Requirement 8.4)
 * - CI integration and coverage (Requirement 8.5)
 */
...
describe('Comprehensive Critical Function Tests', () => {
  beforeEach(() => {
    comprehendMock.reset();
    bedrockMock.reset();
  });

  describe('Requirement 8.1: PII Masking Edge Cases', () => {
    ...
    it('should handle empty and invalid inputs gracefully', async () => {
      // Test empty string
      const emptyResult = await piiService.redactPII('');
      expect(emptyResult.maskedText).toBe('');
      expect(emptyResult.entitiesFound).toHaveLength(0);
  ...
  describe('Requirement 8.2: Knowledge Base Integration', () => {
    ...
    it('should successfully query knowledge base with proper API calls', async () => {
      const mockResponse = {
    ...
    it('should handle Bedrock service errors properly', async () => {
  ...
  describe('Requirement 8.4: Empty Citations and Insufficient Basis Templates', () => {
    let knowledgeBase: BedrockKnowledgeBase;
  ...
  describe('Requirement 8.5: Performance and Integration Testing', () => {
    ...
    it('should handle large text processing efficiently', async () => {
      const piiService = new PiiService();
      const largeText = 'Contact info: ' + 'A'.repeat(10000) + ' test@example.com';
      
      comprehendMock.on(DetectPiiEntitiesCommand).resolves({
        Entities: [
          { Type: 'EMAIL', Score: 0.99, BeginOffset: largeText.indexOf('test@example.com'), EndOffset: largeText.length },
        ],
      });

      const startTime = Date.now();
      const result = await piiService.redactPII(largeText);
      const endTime = Date.now();
